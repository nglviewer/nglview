...
def get_notebooks(args):
    ...
    if args.api:
        ...
        return glob('tests/notebooks/test*ipynb') + glob(
            'tests/notebooks/api/test*ipynb')
...
"""
    all_notebooks = '\n'.join(body_template % (notebook, notebook, n_cells)
                              for (notebook,
                                   n_cells) in notebooks_with_cell_lengths)
...
if __name__ == '__main__':
    ...
    notebooks_with_cell_lengths = [
        (nb, 2 * get_cell_length(nb)) for nb in notebooks
    ]
