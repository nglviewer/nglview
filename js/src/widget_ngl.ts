...
export class NGLView extends widgets.DOMWidgetView {
  ...
  createStage() {
    // init NGL stage
    const stage_params = this.model.get('_ngl_full_stage_parameters');
    ...
    const that = this;
    const width =
      this.model.get('_view_width') || this.$el.parent().width() + 'px';
  ...
  finalizeDisplay() {
    ...
    const state_params = this.stage.getParameters();
    this.model.set('_ngl_original_stage_parameters', state_params);
    this.touch();
    if (
      !this.isEmbeded() &&
      this.stage.compList.length < this.model.get('n_components')
    ) {
      // only call this in notebook to avoid calling handleEmbed twice in embeded mode.
      this.handleEmbed();
    }
    const ngl_view_ids = this.model.get('_ngl_view_id');
  ...
  handleSignals() {
    ...
    this.stage.signals.componentRemoved.add(async function (component) {
      ...
      for (var k in views) {
        ...
        if (
          view.uuid != that.uuid &&
          view.stage.compList.length > n_components
        ) {
    ...
    this.stage.viewerControls.signals.changed.add(() => {
      ...
      if (that._synced_model_ids.length > 0 && that._ngl_focused == 1) {
        that._synced_model_ids.forEach(async mid => {
          const model = await that.model.widget_manager.get_model(mid);
  ...
  async mouseOverDisplay(type) {
    ...
    if (this.btn_pview_fullscreen) {
      const btn = await this.btn_pview_fullscreen;
    ...
    if (this.player_pview) {
      const v = await this.player_pview;
  ...
  updateNGLTheme(css_content) {
    const ele = document.getElementById('nglview_style');
    if (ele != undefined) {
  ...
  _handleEmbedBeforeStage() {
    ...
    const that = this;
    const ngl_color_dict = that.model.get('_ngl_color_dict');
    let label;
  ...
  async handleEmbed() {
    const that = this;
    const ngl_msg_archive = that.model.get('_ngl_msg_archive');
    const ngl_stage_params = that.model.get('_ngl_full_stage_parameters');
    const loadfile_list = [];
    ...
    if (that.model.comm === undefined) {
      const ngl_coordinate_resource = that.model.get(
        '_ngl_coordinate_resource'
      );
      const n_frames = ngl_coordinate_resource['n_frames'] || 1;
      that.model.set('max_frame', n_frames - 1); // trigger updating slider and player's max
      that.touch();
      const model = await that.getPlayerModel();
      const pmodel = model.get('children')[0];
      that.listenTo(pmodel, 'change:value', () => {
        that.updateCoordinatesFromDict(
          ngl_coordinate_resource,
          pmodel.get('value')
        );
      });
    }
    ...
    that.model.get('_ngl_msg_archive').forEach(msg => {
      if (msg.fire_embed) {
        that.on_msg(msg);
      }
    });
  ...
  updateCoordinatesFromDict(cdict, frame_index) {
    ...
    for (let i = 0; i < keys.length; i++) {
      const traj_index = keys[i];
      const coordinates = this.decode_base64(cdict[traj_index][frame_index]);
  ...
  requestUpdateStageParameters() {
    const updated_params = this.stage.getParameters();
    this.model.set('_ngl_full_stage_parameters', updated_params);
    this.touch();
  }

  requestReprParameters(component_index, repr_index) {
    const comp = this.stage.compList[component_index];
    const repr = comp.reprList[repr_index];
  ...
  request_repr_dict() {
    ...
    if (that._synced_repr_model_ids.length > 0) {
      that._synced_repr_model_ids.forEach(async mid => {
        const model = await that.model.widget_manager.get_model(mid);
        for (const k in model.views) {
          const view = await model.views[k];
  ...
  syncReprForAllViews() {
    ...
    if (
      JSON.stringify(repr_dict_frontend) !== JSON.stringify(repr_dict_backend)
    ) {
  ...
  async syncReprWithMe() {
    ...
    for (const k in this.model.views) {
      const v = await this.model.views[k];
      if (v.uuid != that.uuid) {
  ...
  async createView(trait_name) {
    ...
    const model_id = this.model.get(trait_name).replace('IPY_MODEL_', '');
    return await manager.create_view(await manager.get_model(model_id));
  }

  getPlayerModel() {
    // return a Promise
    const model_id = this.model.get('_iplayer').replace('IPY_MODEL_', '');
  ...
  setRepresentation(name, params, component_index, repr_index) {
    ...
    if (repr) {
      ...
      const new_repr = NGL.makeRepresentation(
        name,
        component.structure,
        this.stage.viewer,
        params
  ...
  setColorByResidue(colors, component_index, repr_index) {
    const repr = this.stage.compList[component_index].reprList[repr_index];
  ...
  addShape(name, shapes) {
    ...
    for (let i = 0; i < shapes.length; i++) {
      const shapes_i = shapes[i];
      const shape_type = shapes_i[0];
      const params = shapes_i.slice(1, shapes_i.length);
      // e.g params = ('sphere', [ 0, 0, 9 ], [ 1, 0, 0 ], 1.5)

      const func = shape_dict[shape_type];
  ...
  addBuffer(name, kwargs) {
    ...
    const buffer_class = class_dict[name];
    const buffer = new buffer_class(params);
  ...
  async replaceStructure(structure) {
    ...
    const comp = this.stage.compList[0];
    const representations = comp.reprList.slice();
    const old_orientation = this.stage.viewerControls.getOrientation();
    const component = await this.stage.loadFile(blob, params);
    stage.viewerControls.orient(old_orientation);
    representations.forEach(repr => {
      const repr_name = repr.name;
      const repr_params = repr.repr.getParameters();
  ...
  superpose(cindex0, cindex1, align, sele0, sele1) {
    ...
    const component0 = this.stage.compList[cindex0];
    const component1 = this.stage.compList[cindex1];
  ...
  updateCoordinates(coordinates, model) {
    ...
    const component = this.stage.compList[model];
  ...
  handleResize() {
    ...
    const height = this.$el.height() + 'px';
  ...
  GUIStyleChanged() {
    const style = this.model.get('gui_style');
    if (style === 'ngl') {
      this.createNglGUI();
    } else {
      if (this.stage_widget) {
        ...
        const width = this.$el.parent().width() + 'px';
        const height = this.$el.parent().height() + 'px';
        this.setSize(width, height);
      }
    }
  }
  ...
  parametersChanged() {
    const _parameters = this.model.get('_parameters');
  ...
  async _exportImage(wid, params) {
    if (this.ngl_view_id == this.get_last_child_id()) {
      ...
      reader.onload = function () {
        arr_str = (reader.result as string).replace(
  ...
  _getLoadFilePromise(msg) {
    // args = [{'type': ..., 'data': ...}]
    const args0 = msg.args[0];
    if (args0.type == 'blob') {
      let blob;
      if (args0.binary) {
        const decoded_data = this.decode_base64(args0.data);
  ...
  get_last_child_id() {
    const keys = this.model.get('_ngl_view_id');
  ...
  on_msg(msg) {
    ...
    if (msg.type == 'call_method') {
      let index, component, func, stage;
      const new_args = msg.args.slice();
      new_args.push(msg.kwargs);

      // handle color
      if (
        msg.methodName == 'addRepresentation' &&
        msg.reconstruc_color_scheme
      ) {
        msg.kwargs.color = this.addColorScheme(
          msg.kwargs.color,
          msg.kwargs.color_label
        );
      }
      ...
      const coordinatesDict = msg.data;
